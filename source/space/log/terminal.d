...
int lines()
{
	...
	else version(Windows)
	{
		...
		enforce(GetConsoleScreenBufferInfo(stdoutHandle, &info), sysErrorString(GetLastError()));
		...
		else version(Windows)
		{
			...
			final switch(capability)
			{
				case Capability.foreground:
					characterAttributes = cast(Color)(characterAttributes & ~0x0007) |
					                      WindowsForegroundColor[cast(Color)args];
					break;
				case Capability.background:
					characterAttributes = (characterAttributes & ~0x0070) |
										  WindowsBackgroundColor[cast(Color)args];
					break;
